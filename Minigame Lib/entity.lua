...
function entity:draw()
  ...
  love.graphics.draw(self.image, self.physicsBody:getX()+xModification(), self.physicsBody:getY()+yModification(), self.physicsBody:getAngle(), 1, 1, self.image:getWidth()/2, self.image:getHeight()/2)
end
...
function entity:isHot(x,y)
    ...
    ------------
    if imgX+xModification() < 0 or imgX+xModification()> self.image:getWidth()-1 or imgY+yModification()<0 or imgY+yModification() > self.image:getHeight()-1 then
      return false
    end
    
    local r, g, b, a = self.image:getData():getPixel(imgX-xModification(),imgY-yModification())
    if a ~= 0 then
      return true
    end
  return false
